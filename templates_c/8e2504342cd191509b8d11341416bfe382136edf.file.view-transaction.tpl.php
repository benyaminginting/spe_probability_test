<?php /* Smarty version Smarty-3.0.8, created on 2020-09-02 06:11:44
         compiled from "C:\xampp\htdocs\spe/modules/transaction/view-transaction.tpl" */ ?>
<?php /*%%SmartyHeaderCode:49905f4ed5304a9ec4-49311710%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '8e2504342cd191509b8d11341416bfe382136edf' => 
    array (
      0 => 'C:\\xampp\\htdocs\\spe/modules/transaction/view-transaction.tpl',
      1 => 1599001899,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '49905f4ed5304a9ec4-49311710',
  'function' => 
  array (
  ),
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					field += '<td>'+item.customer_name+'</td>';
					field += '<td>'+item.amount+'</td>';
					field += '<td>'+item.datetime+'</td>';
					field += '<td>' + item.is_disc + '</td>';
					field += '<td>' + item.disc_rate + '</td>';
					field += '<td>' + item.disc_amount + '</td>';
					field += '<td>' + item.pay_amount + '</td>';
					field += '<td>' + item.disc_prob + '</td>';
			...
			if (json.databody == "") {
				$("#tabeldata tbody").append('<tr><td colspan="11" style="text-align:center;">No available data.</td></tr>');
			}
...
function deleteSelectedItems(){
	...
	$.ajax({
	  type : 'POST',
	  url : '<?php echo $_smarty_tpl->getVariable('baseurl')->value;?>
/transaction/query-delete-selected-items',
	  data : {
	    'dataid' : itemToDelete
	  },
	  beforeSend : function(){
