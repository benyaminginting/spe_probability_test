<?php /* Smarty version Smarty-3.0.8, created on 2020-08-31 06:39:36
         compiled from "C:\xampp\htdocs\spe\super/modules/product/view-product.tpl" */ ?>
<?php /*%%SmartyHeaderCode:278985f4c38b89c70a6-43992014%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'f9ac6dbfb32c650d160852fd3db363a912e27dcd' => 
    array (
      0 => 'C:\\xampp\\htdocs\\spe\\super/modules/product/view-product.tpl',
      1 => 1598830375,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '278985f4c38b89c70a6-43992014',
  'function' => 
  array (
  ),
...
<div class="col-md-4" align="left"> 
 ...
 $_from = $_smarty_tpl->getVariable('datacategory')->value; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array');}
if ($_smarty_tpl->_count($_from) > 0){
    foreach ($_from as $_smarty_tpl->tpl_vars['data']->key => $_smarty_tpl->tpl_vars['data']->value){
?>
		        <option value="<?php echo $_smarty_tpl->tpl_vars['data']->value['id'];?>
"><?php echo $_smarty_tpl->tpl_vars['data']->value['parentcategory'];?>
	...
	<div class="row">
		<div class="col-md-12">
			<label class="checkbox-inline">
  				<input type="checkbox" class="cb_new" id="cb_new" value="yes" name="new"> New Products 
	  		</label>
			<label class="checkbox-inline">
				<input type="checkbox" class="cb_featured" id="cb_featured" value="yes" name="featured"> Featured Products  
			</label>
			<label class="checkbox-inline">
				<input type="checkbox"  class="cb_terlaris" id="cb_terlaris" value="yes" name="terlaris"> Best Seller 
			</label>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12">
	      	...
	      	<a type="button" class="btn btn-default" onclick="ajaxpage('<?php echo $_smarty_tpl->getVariable('baseurl')->value;?>
/product/upload', 'Upload Products Data')"><i class="icon-upload"></i>Upload data Produk</a>
      ...
      <div class="row">
        <div class="col-md-4">
	      <select name="pilihfilter" class="form-control" id="pilihfilter">
            ...
            <option value="qty_lt">Qty <= </option>
            <option value="qty_gt">Qty >= </option>
	      </select>
        </div>
        <div class="col-md-8">
...
$('#category').change(izydata);
	...
	$('#cb_new').change(izydata);
	$('#cb_featured').change(izydata);
	$('#cb_terlaris').change(izydata);
	$('#dd_active').change(izydata);
		...
		success : function(r) {	
			...
			if (json.databody != null) {
				$.each(json.databody, function(m, item) {
					...
					if (item.new == 'Y') new_product = 'Yes';
					if (item.featured == 1) featured = 'Yes';
					if (item.terlaris == 1) terlaris = 'Yes';
					if (item.status == 'Y') aktif = 'Yes';
					...
					field += '<td>'+new_product+'</td>';
					field += '<td>'+featured+'</td>';
					field += '<td>'+terlaris+'</td>';
					field += '<td>'+aktif+'</td>';
					field += '<td style="text-align:right;">'+numeral(item.qty).format('0,0')+'</td>';
			...
			if (json.databody == "") {
				$("#tabeldata tbody").append('<tr><td colspan="10" style="text-align:center;">No available data.</td></tr>');
			}
...
/product/export?" + $('#frmpage').serialize() + "&namafile=" + namafile;
  }
}
...
function deleteSelectedItems(){
	...
	$.ajax({
	  type : 'POST',
	  url : '<?php echo $_smarty_tpl->getVariable('baseurl')->value;?>
/product/query-delete-selected-items',
	  data : {
	    'dataid' : itemToDelete
	  },
	  beforeSend : function(){
