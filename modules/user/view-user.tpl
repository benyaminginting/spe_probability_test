...
function editSelectedItems(){
	...
	$.each(items, function(key, value){
	  ...
	  window.open('{$baseurl}/user/edit/'+items[key].value);
	});
}
function deleteSelectedItems(){
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/user/query-delete-selected-items',
	  data : {
	    'dataid' : itemToDelete
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/user/add', 'Add Kategori Item');
}

function editdata(id) {
	ajaxpage('{$baseurl}/user/edit/'+id, 'Edit Kategori Item');
}

function editautorisasi(id){
	$("#modal").modal("show");
	$(".modal-title").text("Edit Autorisasi Gudang");
	$('.modal-body').load("{$baseurl}/user/editautorisasi/" + id);
}
