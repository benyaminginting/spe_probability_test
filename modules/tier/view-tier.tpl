...
function editSelectedItems(){
	...
	$.each(items, function(key, value){
	  ...
	  window.open('{$baseurl}/tier/edit/'+items[key].value);
	});
}
function deleteSelectedItems(){
	...
	$.ajax({
	  type : 'POST',
	  url : '{$baseurl}/tier/query-delete-selected-items',
	  data : {
	    'dataid' : itemToDelete
	  },
	  beforeSend : function(){
...
function tambahdata() {
	ajaxpage('{$baseurl}/tier/add', 'Add Kategori Item');
}

function editdata(id) {
	ajaxpage('{$baseurl}/tier/edit/'+id, 'Edit Kategori Item');
}
